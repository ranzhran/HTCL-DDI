...
def compute_max_bonds(scope: List[Tuple[int, int]]) -> int:
    """Given the scopes for a batch of molecules, computes max bonds."""
    max_bonds = 0
    for st, le in scope:
        if le > max_bonds:
            max_bonds = le
...
def convert_to_3D_bond(input, scope, max_bonds, device, self_attn=True):
    """Converts the input to a 3D batch matrix

    Args:
        input: A tensor of shape [# bonds, # features]
        scope: A list of start/length indices for the molecules
        max_bonds: The maximum number of bonds for padding purposes
        device: For creating tensors
    Returns:
        A matrix of size [batch_size, max bonds, # features]
    """
    n_features = input.size()[1]
    ...
    for (st, le) in scope:
        ...
        n_bonds = le
        n_padding = max_bonds - le

        mask = torch.ones([n_bonds], device=device)
        ...
        mask = mask.repeat([max_bonds, 1]) * mask.unsqueeze(1)
        if not self_attn:
